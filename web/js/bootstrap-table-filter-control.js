...
!function ($) {
    ...
    var createControls = function (that, header) {
        ...
        if (addedFilterControl) {
            ...
            header.off('change focus blur', 'input').on('change focus blur', 'input', function (event) {
                clearTimeout(timeoutId);
                timeoutId = setTimeout(function () {
                    that.onColumnSearch(event);
                }, that.options.searchTimeOut);
            ...
            if (header.find('.date-filter-control').length > 0) {
                $.each(that.columns, function (i, column) {
                    if (column.filterControl !== undefined && column.filterControl.toLowerCase() === 'datepicker') {
                        header.find('.date-filter-control.' + column.field).daterangepicker(column.filterDatepickerOptions);
                    }
                });
            }
        } else {
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        filterTemplate: {
            ...
            datepicker: function (that, field, isVisible) {
                return sprintf(
                    '<input type="text" class="date-filter-control %s form-control" style="width: 100%; visibility: %s">', field, isVisible);
            }
        }
    });
    ...
    BootstrapTable.prototype.initSearch = function () {
        ...
        var filtered_data = $.grep(this.data, function (item, i) {
            for (var key in fp) {
                ...
                if (thisColumn.filterStrictSearch) {
                    ...
                    if( !($.inArray(key, that.header.fields)) !== -1 && thisColumn.filterControl == 'datepicker'){
                        var date_rang_arr = fval.split(' - ');
                        var date_from,date_to;
                        var value_date = DateToUnix(value.split(' ')[0]);
                        if( date_rang_arr.length == 1) {
                            date_from = DateToUnix(date_rang_arr[0]);
                        }else if( date_rang_arr.length == 2) {
                            date_from = DateToUnix(date_rang_arr[0]);
                            date_to = DateToUnix(date_rang_arr[1]);
                        }
                        if( (value_date >= date_from && value_date <= date_to) || value_date == date_from ) {
                            return true;
                        }
                        return false;
                    }else if (!($.inArray(key, that.header.fields) !== -1 &&
                        (typeof value === 'string' || typeof value === 'number') &&
        ...
        this.data = fp ? filtered_data : this.data;
    };
